        ...
        <div class="col-sm-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form class="form-horizontal" method="POST" action="{{ route('product.variant.update', $variant->id) }}" enctype="multipart/form-data">
                        @csrf
                                    ...
                                    <div class="mb-3">
                                        @foreach($variant->images as $img)
                                            <span class="d-inline-block mr-2 mb-2" style="position:relative;">
                                                <img src="{{ asset($img->image_path) }}" alt="" style="max-height:60px; max-width:80px; object-fit:contain; border:1px solid #ddd;">
                                                <a href="{{ route('product.variant.image.delete', $img->id) }}" class="btn btn-xs btn-danger" style="position:absolute; top:-8px; right:-8px;" onclick="return confirm('Remove this image?')">×</a>
                                            </span>
                                        @endforeach
                                    </div>
                                @endif
                                <input type="file" name="images[]" class="form-control" accept="image/*" multiple>
                                <small class="text-muted">Add more images for this variant. Existing images above can be removed with ×.</small>
                            </div>
                        </div>
