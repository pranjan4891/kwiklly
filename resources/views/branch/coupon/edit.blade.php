        ...
        <div class="col-sm-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title">Edit Coupon Form</h3>
                </div>
                ...
                <div class="panel-body">
                    <form action="{{ route('branch.coupons.update', $coupon->id) }}" method="POST">
                        @csrf
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-8">
                                <input type="datetime-local" name="starts_at" class="form-control" value="{{ old('starts_at', \Carbon\Carbon::parse($coupon->starts_at)->format('Y-m-d\TH:i')) }}">
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-8">
                                <input type="datetime-local" name="expires_at" class="form-control" value="{{ old('expires_at', \Carbon\Carbon::parse($coupon->expires_at)->format('Y-m-d\TH:i')) }}">
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-8">
                                <select name="applies_to" class="form-control" required>
                                    ...
                                    <option value="product" {{ old('applies_to', $coupon->applies_to) == 'product' ? 'selected' : '' }}>Specific Products</option>
                                    <option value="category" {{ old('applies_to', $coupon->applies_to) == 'category' ? 'selected' : '' }}>Categories</option>
                                    <option value="subcategory" {{ old('applies_to', $coupon->applies_to) == 'subcategory' ? 'selected' : '' }}>Subcategories</option>
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-lg-8">
                                <select name="category_ids[]" class="form-control select2" multiple="multiple" data-placeholder="Select Categories">
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" {{ in_array($category->id, old('category_ids', $coupon->categories->pluck('id')->toArray())) ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-lg-8">
                                <select name="subcategory_ids[]" class="form-control select2" multiple="multiple" data-placeholder="Select Subcategories">
                                    @foreach($subcategories as $subcategory)
                                        <option value="{{ $subcategory->id }}" {{ in_array($subcategory->id, old('subcategory_ids', $coupon->subcategories->pluck('id')->toArray())) ? 'selected' : '' }}>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-8">
                                <label>
                                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $coupon->is_active) ? 'checked' : '' }}> Active
                                </label>
                            </div>
                        </div>
...
<script>
    $(document).ready(function () {
        ...
        function formatResult(data) {
            ...
            return $('<span><input type="checkbox" style="margin-right:6px;" disabled />' + data.text + '</span>');
        }
