    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="panel panel-default">
                ...
                @endif
                @if($errors->any())
                    <div class="alert alert-danger"><ul class="mb-0"><li>{{ $errors->first() }}</li></ul></div>
                @endif
                <div class="panel-body">
                    @php
                    ...
                    <form class="form-horizontal" method="POST" action="{{ route('vendor.product.variant.store') }}" id="variantForm" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-6"><input type="number" step="0.01" name="variant_actual_price" class="form-control" required></div>
                        </div>
                        <div class="form-group">
                            <label class="col-md-2 control-label">Selling Price (Rs)</label>
                            <div class="col-md-6"><input type="number" step="0.01" name="variant_selling_price" class="form-control" required></div>
                        </div>

                        @if(!$colorAttr)
                        <div class="form-group">
                            <label class="col-md-2 control-label">Stock</label>
                            <div class="col-md-6"><input type="number" name="stock" class="form-control" value="0" required></div>
                        </div>
                        @endif
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-6">
                                <select name="attributes[{{ $attribute->name }}]" class="form-control non-color-attr" required>
                        ...
                        <div id="colorBlocks" class="col-md-12">
                            <div class="color-block">
                                <div class="form-group color-row-inline">
                                    ...
                                    <div class="col-md-3"><input type="file" name="colors[0][images][]" class="form-control" accept="image/*" multiple></div>
                                    <div class="col-md-2">Stock:<input type="number" name="colors[0][stock]" class="form-control color-stock" value="0" min="0" placeholder="Stock"></div>
                                    <div class="col-md-2"><button type="button" class="btn btn-danger btn-sm btn-remove-color btn-color-icon" title="Remove"><i class="fa fa-trash"></i></button></div>
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-md-offset-2 col-md-6">
                                <button type="button" id="addColorBlock" class="btn btn-info btn-color-icon"><i class="fa fa-plus"></i> Add color</button>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-6"><input type="text" name="variant_name" class="form-control" placeholder="e.g. 8/256 ya Default" required></div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-6"><input type="file" name="images[]" class="form-control" accept="image/*" multiple></div>
                        </div>
                            ...
                            @forelse($product->variants as $index => $variant)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if($imgs->isNotEmpty()) {{ $imgs->count() }} image(s) @else <span class="text-muted">—</span> @endif
                                    </td>
                                    <td>
                                        <a href="{{ route('vendor.product.variant.edit', $variant->id) }}" class="btn btn-sm btn-primary">Edit</a>
                                        <form action="{{ route('vendor.product.variant.destroy', $variant->id) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    addBtn.addEventListener('click', function() {
        ...
        if (stockInput) { stockInput.name = 'colors[' + index + '][stock]'; stockInput.value = '0'; }
    ...
    colorBlocks.addEventListener('click', function(e) {
        if (e.target.classList.contains('btn-remove-color')) {
            ...
            if (blocks.length > 1) e.target.closest('.color-block').remove();
        }
    });
    document.querySelectorAll('.non-color-attr').forEach(function(sel) {
        sel.addEventListener('change', function() {
            var parts = [];
            document.querySelectorAll('.non-color-attr').forEach(function(s) { if (s.value) parts.push(s.value); });
