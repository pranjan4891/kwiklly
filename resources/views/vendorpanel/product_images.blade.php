        ...
        <div class="col-sm-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <div class="form">
                        <form class="cmxform form-horizontal tasi-form" method="post" action="{{ route('vendor.product.images.store') }}" enctype="multipart/form-data">
                            @csrf
                                ...
                                <tbody>
                                    @foreach ($productImages as $key => $product)
                                        <tr>
                                           ...
                                           <td>
                                                @if ($isDeletedView)
                                                    <form action="{{ route('vendor.product.images.restore', $product->id) }}" method="POST" style="display:inline-block;">
                                                        @csrf
                                                        <button class="btn btn-warning btn-sm">Restore</button>
                                                    </form>

                                                    <form action="{{ route('vendor.product.images.erase', $product->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Are you sure to permanently delete this product?');">
                                                        @csrf
                                                        @method('DELETE')
                                                ...
                                                @else
                                                    ...
                                                    <a href="{{ route('vendor.product.images.delete', $product->id) }}" class="btn btn-danger" onclick="return confirm('Are you sure to move trash product?');"><i class="fa fa-trash" aria-hidden="true"></i> Trash</a>
                                                @endif
                                            </td>
                                        ...
                                        <div class="modal fade" id="editModal{{ $product->id }}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel{{ $product->id }}" aria-hidden="true">
                                            <div class="modal-dialog" role="document">
                                                <form method="POST" action="{{ route('vendor.product.images.update', $product->id) }}" enctype="multipart/form-data">
                                                    @csrf
                                                            ...
                                                            <div class="row">
                                                                @foreach ($product->product_images as $index => $image)
                                                                    <div class="col-md-3 mb-3 text-center">
                                                                        ...
                                                                        <form method="POST" action="{{ route('vendor.product.image.delete.single') }}">
                                                                            @csrf
