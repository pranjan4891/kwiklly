...
<div class="xyz-modal-overlay" id="couponModalxyz-{{ $vendorId }}" data-vendor-id="{{ $vendorId }}" onclick="if(event.target === this) { if(typeof window.hideModal === 'function') { window.hideModal({{ $vendorId }}); } }" style="display: none; justify-content: center; align-items: center;">
    <div class="xyz-modal" onclick="event.stopPropagation()" style="position: relative; z-index: 1;">
        <!-- Header -->
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 class="mb-0" style="font-size: 18px;"><b>Coupons</b></h5>
            <button type="button" class="xyz-close" onclick="(function() { const vid = {{ $vendorId }}; if(typeof window.hideModal === 'function') { window.hideModal(vid); } else { $('#couponModalxyz-' + vid).fadeOut(200, function() { $(this).css('display', 'none'); }); } })(); return false;" style="font-size: 28px; line-height: 1; padding: 0; background: none; border: none; cursor: pointer; color: #000;">&times;</button>
        </div>
                        ...
                        <div class="col-7">
                            ...
                            <div>
                                <button type="button" class="apply-coupon-btn"
                                    ...
                                    style="background-color: #E94412; color: #fff; font-size: 14px; font-weight: 700; cursor: pointer; display: inline-block; padding: 10px 24px; border: none; border-radius: 50px; transition: all 0.3s ease; box-shadow: 0 2px 8px rgba(233, 68, 18, 0.2);">
                                    Apply
                                </button>
                            </div>
                        </div>
