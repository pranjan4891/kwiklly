...
class HomeController extends Controller
{
    public function index()
    {
        $data['stores'] = VendorAdmin::where('status', '1')
            ...
            ->map(function ($store) {
                if ($store->business_category) {
                    $ids = explode(',', $store->business_category); // your CSV column
                    $store->categories = Category::whereIn('id', $ids)
                        ->select('id', 'name', 'slug')
                        ->get();
                } else {
                    $store->categories = collect(); // empty collection
                }
                return $store;
            });
        $data['categorywiseproducts'] = Category::where('is_deleted', '0')
            ...
            ->map(function ($category) {
                $category->products = Product::with('variants')
    ...
    public function CategoryProducts($category_id, $subcategory_id)
    {
